...
export default function DashboardCliente() {
  ...
  // Obtener solicitudes asociadas al cliente autenticado
  const loadData = useCallback(async () => {
    try {
      const responseSol = await fetch("/api/solicitudes");
      const dataSol = await responseSol.json();

      const misSolicitudes = (dataSol.data ?? []).filter(
        (s) =>
          s.receptor?.nombre?.toLowerCase().trim() ===
          auth.nombre?.toLowerCase().trim()
      );
  ...
  const entregadas = solicitudes.filter(
    (s) => s.estado === "Herramientas entregadas"
  ).length;
  const noEntregadas = solicitudes.filter(
    (s) => s.estado !== "Herramientas entregadas"
  ).length;
          ...
          <div className="bg-white w-full rounded-xl shadow-2xl p-8 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div className="bg-yellow-100 p-4 rounded-lg text-center">
                ...
                <p className="text-sm">Total Solicitudes</p>
              </div>
